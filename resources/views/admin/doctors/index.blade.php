...
@section('content')
    <main class="app sidebar-mini rtl">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-th-list"></i>الاطباء </h1>
            </div>
            <ul class="app-breadcrumb breadcrumb side">
                <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i><a href="{{ route('dashboard') }}"></a>
                </li>

                <li class="breadcrumb-item active"><a href="{{ route('doctors.index') }}" title="الاطباء">الاطباء</a></li>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div style="display: flex; align-items: center; gap: 8px;">
                                                ...
                                                id="status_{{ $doctor->id }}" data-slug="{{ $doctor->slug }}"
                                                {{ $doctor->is_active ? 'checked' : '' }}
                                                style="width: 45px; height: 22px; cursor: pointer;">
                                                @endif
                                                <span
                                                    class="badge {{ $doctor->is_active ? 'badge-success' : 'badge-danger' }}"
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-info" href="{{ route('doctors.show', $doctor) }}"
                                                title="عرض">عرض</a>

                                            <form action="{{ route('doctors.destroy', $doctor) }}" title="حذف"
...
@push('scripts')
    <script type="text/javascript">
        ...
        $('.status-toggle').change(function() {
            ...
            $.ajax({
                url: '{{ route('doctors.toggle-status', ':slug') }}'.replace(':slug', slug),
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    is_active: isActive
